...
type execFunc func(args []string, stdout io.Writer) error
...
type IPTables struct {
	...
	execFn            execFunc    // actually run the iptables command
	commandFn         commandFunc // get the iptables command
...
func Command(fn commandFunc) option {
	return func(ipt *IPTables) {
		ipt.commandFn = fn
	}
}
...
func New(opts ...option) (*IPTables, error) {
	...
	if ipt.commandFn == nil {
		ipt.commandFn = func(args []string) []string {
			return args
		}
	}

	if ipt.execFn == nil {
		ipt.execFn = func(args []string, stdout io.Writer) error {
			...
			if ipt.debug {
				if err != nil {
					fmt.Printf("IPTables ran %s - \nStdOut: %s\n%w\n", cmd.String(), stderr.String(), err)
				} else {
					fmt.Printf("IPTables ran %s - \nStdOut: %s\nStdErr: %s\n", cmd.String(), debugOut, stderr.String())
				}
			}
			if stdout != nil {
				io.Copy(stdout, debugOut)
			}
			if err != nil {
				switch e := err.(type) {
				case *exec.ExitError:
...
func (ipt *IPTables) runWithOutput(args []string, stdout io.Writer) error {
	...
	args = ipt.commandFn(append([]string{ipt.path}, args...))

	if ipt.sudo {
		args = append([]string{"sudo"}, args...)
	}

	return ipt.execFn(args, stdout)
}
