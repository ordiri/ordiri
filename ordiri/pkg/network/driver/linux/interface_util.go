...
func namespaceForDhcp(network api.Network, subnet api.Subnet) string {
	...
	return NetworkServicesNamespacePrefix + network.Tenant() + network.Name() + subnet.Name()
}
...
func publicGwCable(network api.Network) VethCable {
	return VethCable(PublicGatewayCablePrefix + hash(network.Tenant()+network.Name(), IfaceHashSize))
}
func internalRouterCable(network api.Network, subnet api.Subnet, rtr api.Router) VethCable {
	return VethCable(InternalRouterCablePrefix + hash(network.Tenant()+network.Name()+subnet.Name()+rtr.Name(), IfaceHashSize))
}
func servicesCableName(network api.Network, subnet api.Subnet, svc string) VethCable {
	return VethCable(NetworkServiceCablePrefix + hash(network.Tenant()+network.Name()+subnet.Name()+svc, IfaceHashSize))
}
