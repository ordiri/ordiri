...
func renewToken(client *vault.Client) {
	for {
		vaultLoginResp, err := vaultLogin(client)
		if err != nil {
...
func vaultLogin(client *vault.Client) (*vault.Secret, error) {
	loginData := make(map[string]interface{})
	loginData["name"] = "node"

	res, err := client.Logical().Write("auth/cert/login", loginData)
	if err != nil {
		return nil, fmt.Errorf("unable to login: %w", err)
	}

	client.SetToken(res.Auth.ClientToken)
