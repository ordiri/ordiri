...
# this curl won't return a 200 all the time, a 429 indicates healthy still so we don't retry anything with native curl retries
...
while ! curl --retry-all-errors --retry-delay 5 --retry 10 -fsSL -XGET {{vault_addr}}/v1/{{vault_path}}/ca_chain 2>/dev/null; do 
