...
export class VersionApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async getCodeVersion(_options?: Configuration): Promise<RequestContext> {
...
export class VersionApiResponseProcessor {
     ...
     */
     public async getCodeVersion(response: ResponseContext): Promise<IoK8sApimachineryPkgVersionInfo > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: IoK8sApimachineryPkgVersionInfo = ObjectSerializer.deserialize(
